...
class UserController
{
	...
	public function logIn($pseudo, $pass)
	{
		...
		if ($this->verifyPseudoAndPass($pseudo, $pass)) {
			...
			$token = $connexionManager->addTokenConnexion($pseudo);
			// $req2 = $connexionManager->findTokenMatch($token);
			// if ($req2 == true) {
			echo "true|||";
			echo $token;
			// echo $this->token_permission($token);
			// }
			// echo " <br>req <br>";
	...
	public function token_permission($token)
	{
		require_once("model/UserManager.php");
		$connexionManager = new UserManager();
		$req2 = $connexionManager->findTokenMatch($token);
