<template>
  ...
  -->
	<div class="mainSlider">
		...
		<div class="btnPlayPause" v-if="timeslide" @click="timer !== null ? deleteInterval() : userNav(1)">
		...
		<div class="bubbleBox" v-if="bubbles">
			<div v-for="number in initialNbrOfChild" :key="number" :class="bubbleStyle(number)" @click="goToSlide(number)">
...
export default {
	...
	mounted() {
		// if (process.client) {
		this.slidewidthInPx = Math.round(this.$el.clientWidth * (+this.slidewidth / 100));
		this.imgContainer = this.$el.getElementsByClassName("imageContainer")[0];
		...
		for (let i = 0; i < 3; i++) {
			this.imgContainer.insertBefore(this.imgContainer.lastElementChild, this.imgContainer.childNodes[0]);
		}
		...
		this.$el.style.setProperty("--transition-duration", this.slidespeed * 0.8 + "ms");
	...
	methods: {
		...
		setClassName() {
			for (var j = 0; j < this.imgContainer.childNodes.length; j++) {
				if (this.imgContainer.children[j]) {
					...
					this.imgContainer.children[j].classList.remove("going_east");
					this.imgContainer.children[j].classList.remove("going_west");
					// if (this.activeclass) {
					if (j === this.mainImage) {
						// this.imgContainer.children[j].classList.add(this.activeclass);
						this.imgContainer.children[j].classList.add("current_slide");
					} else {
						if (this.imgContainer.children[j].classList.contains("current_slide")) {
							this.imgContainer.children[j].classList.remove("current_slide");
					...
					if (this.unactiveclass) {
						if (j !== this.mainImage) {
							...
							this.imgContainer.children[j].classList.remove(this.unactiveclass);
						}
					}
					if (j > this.mainImage) {
						...
						this.imgContainer.children[j].classList.add("aft" + (j - this.mainImage));
						...
						this.imgContainer.children[j].classList.add("bef" + (this.mainImage - j));
					} else if (j === this.mainImage) {
						if (this.imgContainer.children[j].classList.contains("after")) {
							this.imgContainer.children[j].classList.remove("after");
						}
		...
		goToSlide(imageIndex) {
			...
			while (this.imgContainer.children[3].classList.contains("prems") == false) {
				this.nav(1);
			}
		...
		setHeight() {
			if (this.sliderheight === "very_auto") {
				let imgH;
				try {
					var imageEl = this.imgContainer.children[0].getElementsByTagName("img")[0];
					...
					for (var i = 0; i < this.nbrOfslides; i++) {
						this.imgContainer.children[i].style.height = " var(--current_height)";
					}
				}
			} else {
		...
		animateSlide(direction) {
			if (direction == 1) {
				...
				this.imgContainer.insertBefore(this.imgContainer.lastElementChild, this.imgContainer.children[0]);
				const xx = -this.manualScrollX + this.slidewidthInPx;
				this.imgContainer.animate(
					[
						{
		...
		moveAction(event) {
			if (this.clickeventAction) {
				...
				if (Math.abs(this.startXposition - currentX) > 50 || Math.abs(this.startYposition - currentY) > 50) {
					if (this.startXposition !== -1 || this.startYposition !== -1) {
						const xORy =
							Math.abs(this.startXposition - currentX) > Math.abs(this.startYposition - currentY) ? "x" : "y";

						if (xORy === "x") {
							if (this.startXposition - currentX > 50) {
								this.direction = "E";
								this.imgContainer.getElementsByClassName("current_slide")[0]?.classList.add("going_east");
								this.imgContainer.getElementsByClassName("current_slide")[0]?.classList.remove("going_west");
							} else if (this.startXposition - currentX < -50) {
								this.direction = "O";
								this.imgContainer.getElementsByClassName("current_slide")[0]?.classList.add("going_west");
								this.imgContainer.getElementsByClassName("current_slide")[0]?.classList.remove("going_east");
							}
						} else {
				...
				if (new Date() - this.clickDuration > 10) {
					for (let i = 0; i < this.imgContainer.getElementsByTagName("a").length; i++) {
						this.imgContainer.getElementsByTagName("a")[i].classList.add("noclick");
					}
				}
			}
		},
		...
		stopAction() {
			...
			setTimeout(() => {
				for (let i = 0; i < this.imgContainer.getElementsByTagName("a").length; i++) {
					this.imgContainer.getElementsByTagName("a")[i].classList.remove("noclick");
				}
			}, 100);
